...
export default function BarcodeModalScreen(props) {
    ...
    useEffect(function () {
        let {fromProductDetails} = props.route.params ?? {};

        return (() => {
            if (fromProductDetails) {
                setShouldGetBarcode(true);
                setIsExpectBarcode(true);
            }
        })
    }, [])
    ...
    function onBarcodeScanned(barcode) {
        ...
        if (!isExpectBarcode) {
            Api.post('/GetProductDetails', {
                    strProductCode: barcode.data ? barcode.data : state.barcode,
                    strLoginId: GlobalHelper.loginId,
                    billCustId: props.customer ? props.customer.BillingCustomerId : '',
                    sessionId: props.customer ? props.customer.SessionId : '',
                    cartId: GlobalHelper.cartId
                }
            ).then(resp => {
                setIsLoading(false)
                if (resp.d && resp.d.IsSuccess && resp.d.Product) {
                    props.navigation.replace('ProductDetails', {
                        ...
                        barcodeData: barcode.data ? barcode.data : barcode
                    });
                } else {
                    let msg = "אירעה שגיאה בטעינת תיאור המוצר";
                    if (resp.d.FriendlyMessage) {
                    ...
                    setMessage(Consts.globalErrorMessagePrefix + msg);
                    setShouldGetBarcode(true)
                }
            });
        } else {
            Api.post('/UpdateLineInCart', {
                strProductCode: barcode.data ? barcode.data : barcode, strQuantity: 1, promotionId,
                billCustId: props.customer ? props.customer.BillingCustomerId : '',
                strCartId: GlobalHelper.cartId, strLineId: '', isGetUpdatedCart: false
            }).then(resp => {
                setIsLoading(false)
    ...
    function displayMessageOrBarcode() {
        if (shouldGetBarcode) {
            return <Modal isVisible={true}>
                <View style={styles.modalContent}>
                    <Icon name="barcode" size={40}/>
                    ...
                    <View style={[MySaleStyle.flexRow, MySaleStyle.margTop20]}>
                        <TouchableOpacity onPress={onBarcodeScanned} disabled={!barcode}
            ...
            return <Barcode onBarcodeScanned={onBarcodeScanned}/>;
        }
    }

    return (
        <View style={styles.container}>
            {isLoading ? <MyActivityIndicator/> : displayMessageOrBarcode()}
        </View>
    );
...
exports.BarcodeModalScreenConnectRedux = connect(mapStateToProps, null)(BarcodeModalScreen);
